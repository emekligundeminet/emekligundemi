...
type Props = {
  params: Promise<{ tenantId: string; year: string; month: string }>;
  searchParams: Promise<{ sayfa?: string }>;
};

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { tenantId, year: y, month: m } = await params;
...
export default async function ArsivMonthPage({ params, searchParams }: Props) {
  const { tenantId, year: y, month: m } = await params;
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <FeedPagination basePath={path} page={page} total={feed.total} pageSize={PAGE_SIZE} />
    </div>
  );
}
